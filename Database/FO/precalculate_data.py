...
def get_available_dates():
    try:
        ...
        result = pd.read_sql(
            text(f'SELECT DISTINCT "BizDt" FROM "{sample}" WHERE "BizDt" IS NOT NULL ORDER BY "BizDt" DESC'),
            engine,
        )
        return (
            [d.strftime("%Y-%m-%d") if hasattr(d, "strftime") else str(d) for d in result["BizDt"]]
            if not result.empty
            else []
        )
    except:
...
def calculate_and_store_data(curr_date, prev_date):
    ...
    for table in tables:
        try:
            ...
            closing_price = float(
                dfc["UndrlygPric"].iloc[0] if "UndrlygPric" in dfc.columns else dfc["ClsPric"].iloc[0]
            )
            ...
            for opt_type, prefix in [("CE", "call"), ("PE", "put")]:
                ...
                if not dc.empty and not dp.empty:
                    ...
                    if not dm.empty:
                        ...
                        dm["tradval_chg"] = (dm["OpnIntrst_c"] * dm["LastPric_c"]) - (
                            dm["OpnIntrst_p"] * dm["LastPric_p"]
                        )
                                ...
                                if not dsub.empty:
                                    for metric, data_col in [
                                        ...
                                        if not df_pos.empty:
                                            idx = df_pos[data_col].idxmax()
                                            row_dict[
                                                f"{prefix}_{metric}_pos_strike"
                                            ] = f"{dsub.loc[idx,'StrkPric']:.0f}"

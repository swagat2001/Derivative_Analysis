...
def _load_sector_master():
    ...
    print("[WARNING] No sector master file found. Using classify_industry() only.")
    _sector_cache_loaded = True
        ...
        result.append(
            {
                ...
                "net_index_oi": row[3],
                "total_volume": row[4],
                "sentiment": "Bullish" if row[3] > 0 else "Bearish" if row[3] < 0 else "Neutral",
            }
        )
...
def _get_delivery_data_cached(selected_date: str):
    ...
    try:
        ...
        # Query each table for delivery data
        all_data = []
        batch_size = 50
        ...
        for i in range(0, len(tables), batch_size):
            ...
            if union_parts:
                ...
                try:
                    df_batch = pd.read_sql(text(query), con=engine_cash)
                    all_data.append(df_batch)
                except Exception as e:
                    print(f"[WARN] Delivery batch query failed: {e}")
...
def get_52_week_analysis(selected_date: str):
    ...
    try:
        ...
        # For now, return empty as we don't have 52-week data in options_dashboard_cache
        # This would require querying historical data
        return {"near_high": [], "near_low": [], "at_high": [], "at_low": []}

    except Exception as e:
        print(f"[ERROR] get_52_week_analysis(): {e}")
        return {"near_high": [], "near_low": [], "at_high": [], "at_low": []}

...
def find_latest_data_file():
    """
    Scan folder for existing data files and return the latest date
    Returns: datetime.date object or None if no files found
    """
...
def download_full_bhavcopy():
    ...
    By default: Downloads only NEW data (incremental mode)
    Option to download from scratch if needed
    """
    print("\n" + "=" * 80)
    ...
    latest_date = find_latest_data_file()

    if latest_date:
        print(f"📊 Latest data found: {latest_date.strftime('%d-%m-%Y')}")
        print("\nChoose download mode:")
        print("  1. Download NEW data only (from next day) - RECOMMENDED")
        print("  2. Download from scratch (manual date range)")
        ...
        if mode == "2":
            ...
            # Incremental mode - download from next day after latest
            start_date = latest_date + timedelta(days=1)
            end_date = datetime.now().date()
            print(
                f"\n✅ Incremental mode: Downloading from {start_date.strftime('%d-%m-%Y')} to {end_date.strftime('%d-%m-%Y')}"
            )
    else:
        # No existing data - ask for date range
        print("No existing data found. Enter date range to download:")
        start_date_str = input("Start date (DD-MM-YYYY) [e.g., 22-03-2026]: ").strip()
    ...
    for idx, date_obj in enumerate(date_range, 1):
        ...
        if idx % 10 == 0:
            ...
            print(
                f"{'':12}          Progress: {idx}/{len(date_range)} | "
    ...
    if downloaded_files:
        ...
        for i, file_info in enumerate(display_files, 1):
            ...
            print(
                f"{i:<4} {file_info['date']:<12} {file_info['filename']:<35} "
        ...
        print(
            f"{'ALL':<4} {'':<12} {len(downloaded_files)} files{'':<26} " f"{total_stocks_all:<8} {total_size_str:<10}"
        )

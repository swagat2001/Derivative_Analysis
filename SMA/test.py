...
@st.cache_data(ttl=600)
def calculate_52week_data(available_files, max_days=252):
    """
    Calculate 52-week high/low and position score for all stocks.

    LOGIC:
    1. Load LATEST file to get current prices (files[0])
    2. Iterate through ALL files (including latest) to find high/low
    3. Current price stays fixed from step 1
    """
    if not available_files:
        return pd.DataFrame()

    files = available_files[: min(max_days, len(available_files))]
    stock_data = {}

    # STEP 1: Get current prices from LATEST file ONLY (files[0])
    if files:
        try:
            df_latest = pd.read_csv(files[0]["filepath"], low_memory=False)
            df_latest.columns = df_latest.columns.str.strip()
            df_latest["CLOSE_PRICE"] = pd.to_numeric(df_latest["CLOSE_PRICE"], errors="coerce")

            for _, row in df_latest.iterrows():
                symbol = str(row.get("SYMBOL", "")).strip().upper()
                price = row.get("CLOSE_PRICE")

                if symbol and pd.notna(price):
                    stock_data[symbol] = {"high": price, "low": price, "current": price}
        except Exception as e:
            return pd.DataFrame()

    # STEP 2: Calculate high/low from ALL historical files (including latest)
    for file_info in files:
        try:
            df = pd.read_csv(file_info["filepath"], low_memory=False)
            df.columns = df.columns.str.strip()
            ...
            for _, row in df.iterrows():
                ...
                if symbol and pd.notna(price):
                    if symbol in stock_data:
                        stock_data[symbol]["high"] = max(stock_data[symbol]["high"], price)
                        stock_data[symbol]["low"] = min(stock_data[symbol]["low"], price)
        except:
            continue

    # STEP 3: Calculate position score
    results = []
    for symbol, data in stock_data.items():
        h, l, c = data["high"], data["low"], data["current"]

        if h > l:
            position = ((c - l) / (h - l)) * 100
        else:
            position = 50

        results.append({"SYMBOL": symbol, "52W_HIGH": h, "52W_LOW": l, "CURRENT": c, "POSITION": position})
...
def main():
    ...
    tab_breadth_index, tab_breadth_sector, tab_sector_fund, tab_sma_analysis, tab_two_param, tab_52week = st.tabs(
        [
            "📈 INDEX BREADTH",
            "🏭 SECTOR BREADTH",
            "📊 SECTOR FUNDAMENTALS",
            "📉 SMA ANALYSIS",
            "🔄 TWO-PARAM COMPARISON",
            "📊 52-WEEK HIGH/LOW",
        ]
    )
    ...
    # ===========================================
    # TAB 6: 52-WEEK HIGH/LOW HEATMAP
    # ===========================================
    with tab_52week:
        st.header("📊 52-Week High/Low Stock Analysis")
        st.markdown(f"**Date:** {selected_file['display']}")

        with st.spinner("Calculating 52-week data..."):
            df_52w = calculate_52week_data(available_files)

        if sector_master.empty:
            st.error("❌ Sector master file not found. Place `nse_sector_master.csv` in data folder.")
        elif df_52w.empty:
            st.error("❌ Could not calculate 52-week data.")
        else:
            st.success(f"✅ Analyzed {len(df_52w):,} stocks")
            ...
            merged["POSITION"] = merged["POSITION"].fillna(50)

            # Classify stocks
            merged["AT_52W_HIGH"] = merged["POSITION"] >= 95  # At 52W high (within 5%)
            merged["AT_52W_LOW"] = merged["POSITION"] <= 5  # At 52W low (within 5%)

            st.markdown(
                """
            **Heatmap Shows:**
            - **LEFT**: Number of stocks at **52-Week HIGH** (green intensity)
            - **RIGHT**: Number of stocks at **52-Week LOW** (red intensity)
            """
            )

            # Create sub-tabs for 52-week analysis
            tab_52w_sector, tab_52w_industry, tab_52w_index = st.tabs(["📈 BY SECTOR", "🏭 BY INDUSTRY", "📊 BY INDEX"])

            # ===========================================
            # SUB-TAB 1 - SECTOR
            # ===========================================
            with tab_52w_sector:
                st.subheader("📈 Stocks at 52W High/Low by Sector")

                sector_agg = (
                    merged.dropna(subset=["SECTOR"])
                    .groupby("SECTOR")
                    .agg({"SYMBOL": "count", "AT_52W_HIGH": "sum", "AT_52W_LOW": "sum"})
                    .reset_index()
                )

                sector_agg.columns = ["SECTOR", "TOTAL", "HIGH_COUNT", "LOW_COUNT"]

                sector_agg["HIGH_PCT"] = (sector_agg["HIGH_COUNT"] / sector_agg["TOTAL"] * 100).round(1)
                sector_agg["LOW_PCT"] = (sector_agg["LOW_COUNT"] / sector_agg["TOTAL"] * 100).round(1)

                col_high, col_low = st.columns(2)

                # LEFT: 52W HIGH HEATMAP
                with col_high:
                    st.markdown("#### 🟢 Stocks at 52-Week HIGH")

                    sector_high_sorted = sector_agg.sort_values("HIGH_PCT", ascending=False)

                    sectors_high = sector_high_sorted["SECTOR"].tolist()
                    high_counts = sector_high_sorted["HIGH_COUNT"].tolist()
                    high_pcts = sector_high_sorted["HIGH_PCT"].tolist()
                    totals_high = sector_high_sorted["TOTAL"].tolist()

                    min_high_pct = min(high_pcts) if high_pcts else 0
                    max_high_pct = max(high_pcts) if high_pcts else 5

                    z_data = [[pct] for pct in high_pcts]
                    text_data = [
                        [f"<b>{s}</b><br>{int(count)} stocks ({pct:.1f}%)<br>of {int(total)}"]
                        for s, count, pct, total in zip(sectors_high, high_counts, high_pcts, totals_high)
                    ]
                    ...
                    fig_high = go.Figure(
                        data=go.Heatmap(
                            ...
                            zmin=min_high_pct,
                            zmax=max_high_pct,
                            colorbar=dict(title="% at HIGH", x=1.2, thickness=15, tickformat=".1f", ticksuffix="%"),
                        )
                    )

                    fig_high.update_layout(
                        height=max(500, len(sectors_high) * 50),
                ...
                # RIGHT: 52W LOW HEATMAP
                with col_low:
                    st.markdown("#### 🔴 Stocks at 52-Week LOW")

                    sector_low_sorted = sector_agg.sort_values("LOW_PCT", ascending=False)

                    sectors_low = sector_low_sorted["SECTOR"].tolist()
                    low_counts = sector_low_sorted["LOW_COUNT"].tolist()
                    low_pcts = sector_low_sorted["LOW_PCT"].tolist()
                    totals_low = sector_low_sorted["TOTAL"].tolist()

                    min_low_pct = min(low_pcts) if low_pcts else 0
                    max_low_pct = max(low_pcts) if low_pcts else 5

                    z_data = [[pct] for pct in low_pcts]
                    text_data = [
                        [f"<b>{s}</b><br>{int(count)} stocks ({pct:.1f}%)<br>of {int(total)}"]
                        for s, count, pct, total in zip(sectors_low, low_counts, low_pcts, totals_low)
                    ]
                    ...
                    fig_low = go.Figure(
                        data=go.Heatmap(
                            ...
                            zmin=min_low_pct,
                            zmax=max_low_pct,
                            colorbar=dict(title="% at LOW", x=1.2, thickness=15, tickformat=".1f", ticksuffix="%"),
                        )
                    )

                    fig_low.update_layout(
                        height=max(500, len(sectors_low) * 50),
                        margin=dict(l=5, r=80, t=10, b=10),
                        xaxis=dict(showticklabels=False, showgrid=False),
                        yaxis=dict(showticklabels=False, showgrid=False, autorange="reversed"),
                        plot_bgcolor="white",
                ...
                # Summary table
                with st.expander("📋 View Sector Summary Table with Percentages"):
                    display_df = sector_agg.sort_values("HIGH_PCT", ascending=False)
                    display_df = display_df[
                        ["SECTOR", "TOTAL", "HIGH_COUNT", "HIGH_PCT", "LOW_COUNT", "LOW_PCT"]
                    ].copy()
                    display_df.columns = [
                        "Sector",
                        "Total Stocks",
                    ...
                    display_df["At 52W High (%)"] = display_df["At 52W High (%)"].apply(lambda x: f"{x:.1f}%")
                    display_df["At 52W Low (%)"] = display_df["At 52W Low (%)"].apply(lambda x: f"{x:.1f}%")
                    st.dataframe(display_df, hide_index=True, use_container_width=True)

            # ===========================================
            # SUB-TAB 2 - INDUSTRY
            # ===========================================
            with tab_52w_industry:
                st.subheader("🏭 Stocks at 52W High/Low by Industry")

                industry_agg = (
                    merged.dropna(subset=["NSE_INDUSTRY"])
                    .groupby("NSE_INDUSTRY")
                    .agg({"SYMBOL": "count", "AT_52W_HIGH": "sum", "AT_52W_LOW": "sum"})
                    .reset_index()
                )

                industry_agg.columns = ["INDUSTRY", "TOTAL", "HIGH_COUNT", "LOW_COUNT"]
                industry_agg = industry_agg.sort_values("HIGH_COUNT", ascending=False)
                ...
                with col_high:
                    ...
                    industries = industry_agg["INDUSTRY"].tolist()
                    high_counts = industry_agg["HIGH_COUNT"].tolist()
                    totals = industry_agg["TOTAL"].tolist()

                    max_count = max(high_counts) if high_counts else 1

                    z_data = [[count] for count in high_counts]
                    text_data = [
                        [f"<b>{ind[:25]}</b><br>{int(count)} of {int(total)}"]
                        for ind, count, total in zip(industries, high_counts, totals)
                    ]
                    ...
                    fig_high = go.Figure(
                        data=go.Heatmap(
                            ...
                            colorscale=[[0, "#a8dba8"], [0.3, "#79bd9a"], [0.7, "#3b8686"], [1, "#0b486b"]],
                            showscale=True,
                            xgap=1,
                            ygap=1,
                            zmin=0,
                            zmax=max_count,
                            colorbar=dict(title="Count", x=1.2, thickness=15),
                        )
                    )

                    fig_high.update_layout(
                        height=max(700, len(industries) * 40),
                        margin=dict(l=5, r=80, t=10, b=10),
                        xaxis=dict(showticklabels=False, showgrid=False),
                        yaxis=dict(showticklabels=False, showgrid=False, autorange="reversed"),
                        plot_bgcolor="white",
                ...
                with col_low:
                    ...
                    low_counts = industry_agg["LOW_COUNT"].tolist()
                    max_count_low = max(low_counts) if low_counts else 1

                    z_data = [[count] for count in low_counts]
                    text_data = [
                        [f"<b>{ind[:25]}</b><br>{int(count)} of {int(total)}"]
                        for ind, count, total in zip(industries, low_counts, totals)
                    ]
                    ...
                    fig_low = go.Figure(
                        data=go.Heatmap(
                            ...
                            texttemplate="%{text}",
                            textfont={"size": 9, "color": "black", "family": "Arial Black"},
                            colorscale=[[0, "#ffcccc"], [0.3, "#ff9999"], [0.7, "#ff4444"], [1, "#cc0000"]],
                            showscale=True,
                            xgap=1,
                            ygap=1,
                            zmin=0,
                ...
                with st.expander("📋 View Industry Summary Table"):
                    display_df = industry_agg[["INDUSTRY", "TOTAL", "HIGH_COUNT", "LOW_COUNT"]].copy()
                    display_df.columns = ["Industry", "Total Stocks", "At 52W High", "At 52W Low"]
                    st.dataframe(display_df, hide_index=True, use_container_width=True)
            ...
            # ===========================================
            with tab_52w_index:
                st.subheader("📊 Stocks at 52W High/Low by Index")

                idx_data = []

                for idx_name in ["NIFTY50", "NIFTY100", "NIFTY500", "NIFTYMIDCAP150", "NIFTYSMALLCAP250"]:
                    idx_stocks = merged[merged["INDEX_MEMBERSHIP"].str.contains(idx_name, na=False)]

                    if len(idx_stocks) > 0:
                        total = len(idx_stocks)
                        high_count = idx_stocks["AT_52W_HIGH"].sum()
                        low_count = idx_stocks["AT_52W_LOW"].sum()

                        idx_data.append(
                            {"INDEX": idx_name, "TOTAL": total, "HIGH_COUNT": high_count, "LOW_COUNT": low_count}
                        )

                idx_agg = pd.DataFrame(idx_data).sort_values("HIGH_COUNT", ascending=False)
                ...
                with col_high:
                    ...
                    indices = idx_agg["INDEX"].tolist()
                    high_counts = idx_agg["HIGH_COUNT"].tolist()
                    totals = idx_agg["TOTAL"].tolist()

                    max_count = max(high_counts) if high_counts else 1

                    z_data = [[count] for count in high_counts]
                    text_data = [
                        [f"<b>{idx}</b><br>{int(count)} stocks<br>of {int(total)}"]
                        for idx, count, total in zip(indices, high_counts, totals)
                    ]
                ...
                with col_low:
                    ...
                    low_counts = idx_agg["LOW_COUNT"].tolist()
                    max_count_low = max(low_counts) if low_counts else 1

                    z_data = [[count] for count in low_counts]
                    text_data = [
                        [f"<b>{idx}</b><br>{int(count)} stocks<br>of {int(total)}"]
                        for idx, count, total in zip(indices, low_counts, totals)
                    ]
                ...
                with st.expander("📋 View Index Summary Table"):
                    display_df = idx_agg[["INDEX", "TOTAL", "HIGH_COUNT", "LOW_COUNT"]].copy()
                    display_df.columns = ["Index", "Total Stocks", "At 52W High", "At 52W Low"]
                    st.dataframe(display_df, hide_index=True, use_container_width=True)

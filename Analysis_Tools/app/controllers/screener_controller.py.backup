...
<div class="content">
    ...
    <table style="width:100%;"><tr>
        <td style="width:33%; vertical-align:top; padding:4px;">"""
            + make_heatmap_table(rsi_overbought, "RSI > 80 (Overbought)", "RSI", "#7c3aed")
            + """</td>
        <td style="width:33%; vertical-align:top; padding:4px;">"""
            + make_heatmap_table(rsi_strong_bullish, "RSI 60-80 (Strong Bullish)", "RSI", "#16a34a")
            + """</td>
        <td style="width:33%; vertical-align:top; padding:4px;">"""
            + make_heatmap_table(rsi_mild_bullish, "RSI 50-60 (Mild Bullish)", "RSI", "#4ade80")
            + """</td>
    </tr></table>
    <table style="width:100%; margin-top:5px;"><tr>
        <td style="width:33%; vertical-align:top; padding:4px;">"""
            + make_heatmap_table(rsi_mild_bearish, "RSI 40-50 (Mild Bearish)", "RSI", "#fb923c")
            + """</td>
        <td style="width:33%; vertical-align:top; padding:4px;">"""
            + make_heatmap_table(rsi_strong_bearish, "RSI 20-40 (Strong Bearish)", "RSI", "#dc2626")
            + """</td>
        <td style="width:33%; vertical-align:top; padding:4px;">"""
            + make_heatmap_table(rsi_oversold, "RSI < 20 (Oversold)", "RSI", "#8B2432")
            + """</td>
    </tr></table>
</div>
...
<div class="content">
    ...
    <table style="width:100%;"><tr>
        <td style="width:50%; vertical-align:top; padding:8px;">"""
            + make_heatmap_table(macd_bullish, "MACD > Signal (Bullish)", "MACD", "#16a34a", 20)
            + """</td>
        <td style="width:50%; vertical-align:top; padding:8px;">"""
            + make_heatmap_table(macd_bearish, "MACD < Signal (Bearish)", "MACD", "#dc2626", 20)
            + """</td>
    </tr></table>
    <div style="font-size:11pt; font-weight:bold; color:#8B2432; margin:12px 0 8px 0;">SMA Analysis (200-Day)</div>
    <table style="width:100%;"><tr>
        <td style="width:50%; vertical-align:top; padding:8px;">"""
            + make_heatmap_table(sma_bullish, "Above 200 SMA (Bullish)", "% from SMA", "#16a34a", 20)
            + """</td>
        <td style="width:50%; vertical-align:top; padding:8px;">"""
            + make_heatmap_table(sma_bearish, "Below 200 SMA (Bearish)", "% from SMA", "#dc2626", 20)
            + """</td>
    </tr></table>
    <div style="font-size:11pt; font-weight:bold; color:#8B2432; margin:12px 0 8px 0;">ADX Analysis (Trend Strength)</div>
    <table style="width:100%;"><tr>
        <td style="width:50%; vertical-align:top; padding:8px;">"""
            + make_heatmap_table(adx_strong, "ADX > 25 (Strong Trend)", "ADX", "#16a34a", 20)
            + """</td>
        <td style="width:50%; vertical-align:top; padding:8px;">"""
            + make_heatmap_table(adx_weak, "ADX < 25 (Weak/No Trend)", "ADX", "#dc2626", 20)
            + """</td>
    </tr></table>
</div>"""
        )
...
def create_screener_pdf(screener_data, selected_date):
    ...
    try:
        ...
        replacements = {
            ...
            "{{moneyness_call_itm_gainers_table}}": make_mini_table(
                safe("moneyness_call_itm_gainers"), "Top 10 ITM Moneyness Call Gainers"
            ),
            "{{moneyness_call_otm_gainers_table}}": make_mini_table(
                safe("moneyness_call_otm_gainers"), "Top 10 OTM Moneyness Call Gainers"
            ),
            "{{moneyness_call_losers_table}}": make_mini_table(
                safe("moneyness_call_losers"), "Top 10 Moneyness Call Losers (ALL)"
            ),
            "{{moneyness_call_itm_losers_table}}": make_mini_table(
                safe("moneyness_call_itm_losers"), "Top 10 ITM Moneyness Call Losers"
            ),
            ...
            "{{moneyness_put_itm_gainers_table}}": make_mini_table(
                safe("moneyness_put_itm_gainers"), "Top 10 ITM Moneyness Put Gainers"
            ),
            "{{moneyness_put_otm_gainers_table}}": make_mini_table(
                safe("moneyness_put_otm_gainers"), "Top 10 OTM Moneyness Put Gainers"
            ),
            "{{moneyness_put_losers_table}}": make_mini_table(
                safe("moneyness_put_losers"), "Top 10 Moneyness Put Losers (ALL)"
            ),
            "{{moneyness_put_itm_losers_table}}": make_mini_table(
                safe("moneyness_put_itm_losers"), "Top 10 ITM Moneyness Put Losers"
            ),
            ...
            "{{future_oi_losers_table}}": make_mini_table(
                safe("future_oi_losers"), "Top 10 Future OI Losers", is_future=True
            ),
            "{{future_moneyness_gainers_table}}": make_mini_table(
                safe("future_moneyness_gainers"), "Top 10 Future Moneyness Gainers", is_future=True
            ),
            "{{future_moneyness_losers_table}}": make_mini_table(
                safe("future_moneyness_losers"), "Top 10 Future Moneyness Losers", is_future=True
            ),
        }
        ...
        if signal_analysis_data:
            ...
            for i, (ticker, data) in enumerate(sorted_signals[:5], 1):
                print(
                    f"  {i}. {ticker}: {data['bullish_count']} green, {data['bearish_count']} red, score={data['score']}"
                )
        ...
        print(
            f"[DEBUG] Placeholder replaced. '{{{{technical_heatmap_section}}}}' in tables_html: {'{{technical_heatmap_section}}' in tables_html}"
        )

...
def get_screener_data(selected_date: str, metric_type: str, option_type: str, moneyness_filter: str = "ALL"):
    ...
    if not hasattr(get_screener_data, "_cache") or get_screener_data._cache.get("key") != cache_key:
        get_screener_data._cache = {"key": cache_key, "data": get_all_screener_data(selected_date)}
...
def get_futures_oi_screeners(selected_date):
    ...
    try:
        ...
        for ticker, info in ticker_data.items():
            info["cumulative_oi"] = info.get("cme_oi", 0) + info.get("nme_oi", 0) + info.get("fme_oi", 0)
            data.append(info)
        ...
        for item in data:
            item["cum_oi_percentile"] = (
                sum(1 for c in cum_ois if c < item["cumulative_oi"]) / len(cum_ois) * 100 if cum_ois else 50
            )

        # Organize into 8 categories
        result = {
            "cme_exposure_gainers": sorted(
                [d for d in data], key=lambda x: x.get("cme_exposure_percentile", 0), reverse=True
            )[:10],
            "cme_exposure_losers": sorted([d for d in data], key=lambda x: x.get("cme_exposure_percentile", 0))[:10],
            "nme_exposure_gainers": sorted(
                [d for d in data], key=lambda x: x.get("nme_exposure_percentile", 0), reverse=True
            )[:10],
            "nme_exposure_losers": sorted([d for d in data], key=lambda x: x.get("nme_exposure_percentile", 0))[:10],
            "fme_exposure_gainers": sorted(
                [d for d in data], key=lambda x: x.get("fme_exposure_percentile", 0), reverse=True
            )[:10],
...
def get_technical_indicators_screeners(selected_date):
    ...
    try:
        ...
        for item in data:
            item["strength_percentile"] = (
                sum(1 for s in scores if s < item["composite_score"]) / len(scores) * 100 if scores else 50
            )
        ...
        result = {
            ...
            "macd_bullish": sorted(
                [d for d in data if d["macd"] > d["macd_signal"]],
                key=lambda x: abs(x["macd"] - x["macd_signal"]),
                reverse=True,
            )[:10],
            "macd_bearish": sorted(
                [d for d in data if d["macd"] < d["macd_signal"]],
            ...
            "above_sma": sorted(
                [d for d in data if d["close_price"] > d["sma_50"]],
                key=lambda x: (x["close_price"] - x["sma_50"]) / x["sma_50"] if x["sma_50"] > 0 else 0,
                reverse=True,
            )[:10],
            "below_sma": sorted(
                [d for d in data if d["close_price"] < d["sma_50"]],
                key=lambda x: (x["sma_50"] - x["close_price"]) / x["sma_50"] if x["sma_50"] > 0 else 0,
                reverse=True,
            )[:10],

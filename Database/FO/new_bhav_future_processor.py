...
def greeks(premium, expiry, cd, asset_price, strike_price, intrest_rate, instrument_type):
    try:
        t = (
            (datetime(expiry.year, expiry.month, expiry.day, 15, 30) - datetime(cd.year, cd.month, cd.day, 15, 30))
            / timedelta(days=1)
        ) / 365
...
for date_idx, date_to_process in enumerate(dates_to_process, 1):
    ...
    for table_name in ticker_tables:
        try:
            ...
            df["chg_price"] = round(
                100 * ((df["PrvsClsgPric"] - df["LastPric"]) / ((df["PrvsClsgPric"] + df["LastPric"]) / 2)),
                2,
            )
            ...
            def safe_greeks(row):
                try:
                    if (
                        ...
                        expiry_dt = (
                            datetime.combine(row.get("FininstrmActlXpryDt"), datetime.min.time())
                            if isinstance(row.get("FininstrmActlXpryDt"), datetime.date)
                            else row.get("FininstrmActlXpryDt")
                        )
                        bizdt = (
                            datetime.combine(row.get("BizDt"), datetime.min.time())
                            if isinstance(row.get("BizDt"), datetime.date)
                            else row.get("BizDt")
                        )
